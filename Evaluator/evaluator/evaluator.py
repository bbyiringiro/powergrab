...
# 	for item in day_to_test:
# 		yield (day_to_test[item], item, 2019)
...
def run():
	...
	for day, month, year in date_generator():
		...
		check_call([
			"java", 
			"-jar", 
			"../out/artifacts/powergrab-0.0.1-SNAPSHOT.jar", 
		...
		if int(coinsum) != int(coins):
			print("\n\t!!! %4d out of %4d  (%02.2f)" % (int(coins), int(coinsum), coins / coinsum * 100))

		
		total_coins += coins
		expected_totatl +=  coinsum
		print(" {} in {}".format(total_coins, expected_totatl))
		print(total_coins/expected_totatl*100)
		print()
